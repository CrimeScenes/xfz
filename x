...
local TargetPlayer = nil  -- The specific player you are targeting
...
local function isPlayerKnocked(player)
    ...
    if humanoid then
        return humanoid.Health > 0 and humanoid.Health < 5  -- Example threshold for "knocked" state
    end
    return false
end

local function TriggerBotAction()
    if TargetPlayer == nil then return end  -- Only execute if there is a target
    ...
    if TargetPlayer.Character then
        ...
        if humanoid and humanoid.Health > 0 and not isPlayerKnocked(TargetPlayer) then  -- Check target's health
...
RunService.RenderStepped:Connect(function()
    if TargetPlayer and TargetPlayer.Character and TargetPlayer.Character:FindFirstChild("Humanoid") then
        if TargetPlayer.Character.Humanoid.Health < 5 then
            TargetPlayer = nil  -- Untarget if health is below threshold
            IsTargeting = false
        else
            TriggerBotAction()  -- Regularly trigger the action if the target is still valid
        end
    end
end)

-- Main Loop for Silent feature
    ...
    if getgenv().Silent.Startup.Enabled and IsTargeting then  -- Ensure Silent is enabled and CamLock is engaged
        ...
        if getgenv().Silent.Startup.mode == "target" and TargetPlayer then
            ...
            if target and target.Character then
                -- Check if the target is knocked out (under 5 health)
                if target.Character.Humanoid.Health < 5 then
                    TargetPlayer = nil
                    IsTargeting = false
                    return
                end
        ...
        elseif getgenv().Silent.Startup.mode == "normal" then
            ...
            if target and target.Character then
                -- Check if the target is knocked out (under 5 health)
                if target.Character.Humanoid.Health < 5 then
                    return  -- Skip the shooting logic if target's health is under 5
                end
